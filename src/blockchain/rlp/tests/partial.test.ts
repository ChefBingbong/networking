...
describe("partial", function () {
	describe("RLP encoding:", function () {
		it("should return itself if single byte and less than 0x7f:", function () {
			...
			expect(encodedSelf.equals(RLP.encode(source))).toBe(true);
		});
		...
		it("length of string 0-55 should return (0x80+len(data)) plus data", function () {
			...
			expect(encodedDog.equals(RLP.encode(source))).toBe(true);
		});
		...
		it("length of string >55 should return 0xb7+len(len(data)) plus len(data) plus data", function () {
			...
			expect(encodedLongString.equals(RLP.encode(source))).toBe(true);
		});
		...
		it("encodes `[null]`", function () {
			...
			expect(encodedNull.equals(RLP.encode(source))).toBe(true);
		});
		...
		it("should use a provided `offset` value and `alloc` function", function () {
			...
			expect(encoded.subarray(0, offset).equals(expected)).toBe(true);
			expect(encoded.subarray(offset).equals(RLP.encode(source))).toBe(true);
		});
	});
	describe("nested", () => {
		const source = [
		...
		it("encodes range of nested parts", function () {
			...
			const encodedNull = RLP.digest([start.output], start.length);
			expect(encodedNull.equals(RLP.encode(source.slice(2, 4)))).toBe(true);
		});
		...
		it("digests multiple ranges", function () {
			...
			expect(encodedNull.equals(RLP.encode(source.slice(1)))).toBe(true);
		});
	});
});
