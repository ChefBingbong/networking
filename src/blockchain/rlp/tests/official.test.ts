...
describe("offical tests", function () {
	const fixturePath = join(__dirname, "./fixture/rlptest.json");
	const officalTests = JSON.parse(readFileSync(fixturePath, "utf-8")).tests;

	for (const testName in officalTests) {
		it(`should pass ${testName}`, function () {
			...
			expect("0x" + encoded.toString("hex")).toBe(
