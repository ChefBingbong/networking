...
import { deriveKeys, generateECDH } from "./crypto";

export interface PeerInfo { id: string; host: string; port: number; }
...
export function createServer(
  ...
  const server = net.createServer((sock) => onConn(new MuxedConnection(sock), sock));
...
export async function connect(
  ...
  return new Promise((resolve, reject) => {
    const sock = net.createConnection({ host: target.host, port: target.port }, async () => {
  ...
  const handshakeP = new Promise<void>((res) => {
    mc.setOnFrame((f: Frame) => {
      if (f.t === "HELLO") {
        ...
        mc.send({ t: "HELLO_ACK", payload: { pub: myPub, id: ctx.id, v: PROTOCOL_VERSION } });
    ...
    if (isInitiator) {
      console.log(`[${ctx.id}] -> HELLO`);
      mc.send({ t: "HELLO", payload: { pub: myPub, id: ctx.id, v: PROTOCOL_VERSION } });
    }
  });
