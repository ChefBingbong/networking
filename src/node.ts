...
(async () => {
	...
	console.log(
		`[${ctx.id}] dialing bootstrap ${BOOTSTRAP_HOST}:${BOOTSTRAP_PORT}`,
	);
	const bs = await connect(
		ctx,
		{ id: "bootstrap", host: BOOTSTRAP_HOST, port: BOOTSTRAP_PORT },
		true,
	);
	...
	bs.send({
		t: "PEER_JOIN",
		payload: { id: ctx.id, host: ctx.host, port: ctx.port },
	});
...
function onBootstrapFrame(f: Frame) {
	...
	if (f.t === "PEER_LIST") {
		...
		console.log(
			`[${ctx.id}] received peer list:`,
			[...ctx.peers.keys()].join(", ") || "(none)",
		);
	}
	if (f.t === "PEER_JOIN") {
