...
import { bigIntToBytes, equalsBytes } from "../../utils/index.ts";
...
export function createBlockHeaderFromBytesArray(
	...
	if (header.common.isActivatedEIP(1559) && baseFeePerGas === undefined) {
		...
		if (
			...
			throw Error("invalid header. baseFeePerGas should be provided");
		}
	}
	if (header.common.isActivatedEIP(4844)) {
		if (excessBlobGas === undefined) {
			throw Error("invalid header. excessBlobGas should be provided");
		} else if (blobGasUsed === undefined) {
			throw Error("invalid header. blobGasUsed should be provided");
		}
	}
	if (
		...
		throw Error("invalid header. parentBeaconBlockRoot should be provided");
	}

	if (header.common.isActivatedEIP(7685) && requestsHash === undefined) {
		throw Error("invalid header. requestsHash should be provided");
	}
	return header;
}
...
export function createBlockHeaderFromRLP(
	...
	if (!Array.isArray(values)) {
		throw Error("Invalid serialized header input. Must be array");
	}
