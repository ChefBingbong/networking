...
export class UdpTransport implements KademliaTransport {
  ...
  constructor(
    ...
    const createSocket = options.createSocket ?? (() => dgram.createSocket({ type: 'udp4' }))
    this._socket = createSocket()

    if (this._socket) {
      this._socket.once('listening', () => this.events.emit('listening', undefined))
   ...
   */
  private _send(peer: PeerInfo, typename: MessageTypeName, data: any): Uint8Array {
    if (this.DEBUG) {
      debug(
  ...
  private _handleMessage(msg: Uint8Array, rinfo: dgram.RemoteInfo): void {
    ...
    try {
      ...
      if (this.DEBUG) {
        debug(`failed to decode message from ${rinfo.address}:${rinfo.port}: ${err}`)
      }
      return
    }
