...
export class Transport {
  ...
  listen(
   ...
   this.server = net.createServer((sock) => {
      ...
      try {
        ...
        sock.destroy();
        log(`[${ctx.id}] onConn handler threw: ${String(err)}`);
      }
    });
  ...
  dial(ctx: NodeContext, target: PeerInfo, timeoutMs = 10_000): Promise<MuxedConnection> {
    return new Promise((resolve, reject) => {
      ...
      const onConnect = () => {
        ...
        const mc = new MuxedConnection(ctx, sock);
        log(`[${ctx.id}] connected to ${target.id} at ${target.host}:${target.port}`);
        resolve(mc);
      };
      ...
      const onError = (err: any) => {
        ...
        if (!connected) {
          ...
          log(`[${ctx.id}] post-connect error to ${target.id}: ${err?.message || err}`);
        }
      };
...
function wrapConnErr(target: PeerInfo, err: any): Error {
  ...
  const e = new Error(`connection error ${target.host}:${target.port} - ${msg}`);
  (e as any).cause = err;
  return e;
}
